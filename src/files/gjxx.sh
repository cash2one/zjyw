...
def gjxx_view(request):
    try:
        ...
        # 获取任务信息
        sql = "select * from AgentWarn where warn_isclose='1'  "
...
def pb_gjxx_view(request):
    try:
        ...
        # 获取任务信息
        sql = "update AgentWarn set warn_isclose='0' where warn_id='%s'" % id
...
def gjxx_pb_view(request):
    try:
        ...
        # 获取任务信息
        sql = "select * from AgentWarn where warn_isclose='0'  "
        with myapi.connection() as con:
            cur = con.cursor()
            rs = myapi.sql_execute(cur, sql)
            while rs.next():
...
def no_pb_gjxx_view(request):
    try:
        ...
        id = request.POST.get('id','').encode('utf8')
        # 获取任务信息
        sql = "update AgentWarn set warn_isclose='1' where warn_id='%s'" % id
        ...
        return HttpResponse( json.dumps( content) , content_type='application/json' )
#TODO 异步返回相应结果并更新数据库 task_start、task_stop、task_add、task_edit、task_del
